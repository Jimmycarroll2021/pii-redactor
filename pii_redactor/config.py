...
class Config:
    ...
    # "transformers_au_gliner" (Apache-2.0 urchade/gliner_multi_pii-v1 substrate)
    backend: str = "mock"
    ...
    audit_encryption_key: str | None = None  # Fernet key. None disables encryption.
    ...
    def from_env(cls) -> Config:
        ...
        return cls(
            ...
            llama_cpp_model_name=os.environ.get("PIIR_LLAMA_CPP_MODEL", "llama-3-8b-instruct"),
            hf_model_id=os.environ.get("PIIR_HF_MODEL", "meta-llama/Meta-Llama-3.1-8B-Instruct"),
            ...
            fail_on_llm_error=os.environ.get("PIIR_FAIL_ON_LLM_ERROR", "false").lower() == "true",
            audit_log_path=os.environ.get("PIIR_AUDIT_PATH", "./audit.jsonl"),
            audit_encryption_key=os.environ.get("PIIR_AUDIT_KEY"),
            audit_enabled=os.environ.get("PIIR_AUDIT_ENABLED", "true").lower() == "true",
